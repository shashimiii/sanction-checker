...
from sanctions_engine import DATA_DIR, OUTPUT_DIR, build_report, ensure_dirs, load_sanctions_index
    ...
    try:
        ...
        if not sanctions_index:
            return render_template(
                ...
                error=(
                    "制裁名单加载失败：未读取到任何数据。"
                    "请检查网络连接，或先手工准备 data/ofac_sdn.csv、data/un_consolidated.xml、data/eu_fsf.csv。"
                ),
            )
        result = build_report(names, sanctions_index, run_id=upload_id)
    except Exception as exc:
        traceback.print_exc()
        return render_template("index.html", error=f"执行查询失败: {exc}")
...
def handle_exception(exc: Exception):
    traceback.print_exc()
    return (
        render_template("index.html", error=f"服务异常: {exc}. 请检查依赖是否安装，或查看控制台日志。"),
        500,
    )
