...
def _safe_load(label: str, fn) -> list[SanctionEntry]:
    try:
        return fn()
    except Exception as exc:
        print(f"[WARN] {label} 解析失败: {exc}")
        return []
...
def _best_match(name: str, entries: Iterable[SanctionEntry]) -> tuple[SanctionEntry | None, float]:
    ...
    best_entry = None
    for e in entries:
        target = _normalize(e.name)
        if not target:
            continue
        score = float(fuzz.WRatio(normalized_query, target))
        if score > best_score:
            best_score = score
            best_entry = e
...
def build_report(company_names: list[str], sanctions_index: list[SanctionEntry], run_id: str) -> dict:
    ...
    for i, name in enumerate(company_names, start=1):
        ...
        safe = re.sub(r"[^0-9A-Za-z一-龥_-]+", "_", name)[:40] or f"row_{i}"
